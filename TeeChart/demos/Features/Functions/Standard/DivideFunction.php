<?php
        ...
        $chart1->getChart()->getHeader()->setText("Divide Function");
        ...
        $line->getMarks()->setVisible(false); // Formated text decimals if true
        $divide = new Divide();
        $divide->setChart($chart1->getChart());
        $line->setDataSource(Array($bar1,$bar2));
        $line->setColor(new Color(237,28,36));
        $line->setFunction($divide);

        $chart1->render("chart1.png");
        $rand=rand();
        print '<font face="Verdana" size="2">Divide Function Chart<p>';
...
<head>
  <title>Divide Function Charts</title>
